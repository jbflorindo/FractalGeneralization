...
def get_all_measures(model, init_model, dataloader, 
  ...
  def _margin(
    ...
    for data, target in dataloader:
      ...
      logits = model(data)
      #pdb.set_trace()
      correct_logit = logits[torch.arange(logits.shape[0]), target.long()].clone() # target.long() to fix the error IndexError: tensors used as indices must be long, byte or bool tensors
      logits[torch.arange(logits.shape[0]), target.long()] = float('-inf')
