...
class Network_SingleTask(nn.Module):
    def __init__(self, n_channels, n_classes, batch_norm=True, dropout=True, nbr_blocks=5):
        ...
        self.Conv_Block.append(DoubleConvPooling(n_channels, 32, batch_norm))
        self.Conv_Block.append(DoubleConvPooling(32, 64, batch_norm))
        self.Conv_Block.append(DoubleConvPooling(64, 128, batch_norm))
        self.Conv_Block.append(DoubleConvPooling(128, 256, batch_norm))
        self.Conv_Block.append(DoubleConvPooling(256, 512, batch_norm))
        self.Conv_Block.append(DoubleConv(512, 1024, batch_norm))
        self.Conv_Block.append(DoubleConv(1024, 2048, batch_norm))
        self.Conv_Block.append(DoubleConv(2048, 2048, batch_norm))
        
        dense_size = [[64,32,16],[128,64,32],[256,128,64],[512,256,64],[1024,512,64],[2048,512,64],[2048,512,64]]
        
        # Classification head
        self.Classification_Head = ClassifyLayers(dense_size[nbr_blocks-2][0], dense_size[nbr_blocks-2][1], dense_size[nbr_blocks-2][2], n_classes, dropout)

    def forward(self, x):
        # Encoder
        for i in range(self.nbr_blocks):
...
class DoubleConvPooling(nn.Module):
    # Two convolutional layers: (in_channel size, out_channels size, kernel_size, stride, padding)
    def __init__(self, in_channels, out_channels, batch_norm=True):
        super().__init__()
